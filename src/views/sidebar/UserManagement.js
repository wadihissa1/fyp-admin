...
const UserManagement = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await fetch('http://172.20.10.3:7210/api/User/admin')
        if (!response.ok) {
  ...
  const updateUserDetails = async (userId) => {
    ...
    try {
      const response = await fetch(`http://172.20.10.3:7210/api/User/${userId}`, {
        method: 'PUT',
        headers: {
  ...
  const handleStatusChange = async (userId) => {
    try {
      const response = await fetch(`http://172.20.10.3:7210/api/User/${userId}/toggle-status`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
